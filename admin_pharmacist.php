...
pms_log('Hệ thống', 'Truy cập tính năng cũ: admin_pharmacist.php');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        $existing = pms_fetch_one("SELECT pharmacist_id FROM pharmacist WHERE username = ? AND pharmacist_id <> ? LIMIT 1", 'si', [$username, $id]);
        if ($existing) $errors[] = 'Tên đăng nhập đã tồn tại.';
    }

    if (!$errors) {
        if ($id > 0) {
            $current = pms_fetch_one("SELECT password FROM pharmacist WHERE pharmacist_id = ?", 'i', [$id]);
            $finalPassword = $current['password'] ?? '';
            if ($password !== '') $finalPassword = password_hash($password, PASSWORD_DEFAULT);
            pms_exec("UPDATE pharmacist SET first_name=?, last_name=?, staff_id=?, postal_address=?, phone=?, email=?, username=?, password=? WHERE pharmacist_id=?", 'ssssssssi', [$first, $last, $staff, $address, $phone, $email, $username, $finalPassword, $id]);
            pms_flash('Đã cập nhật tài khoản dược sĩ.');
        } else {
            if ($password === '') $errors[] = 'Mật khẩu là bắt buộc khi tạo tài khoản mới.';
            else {
                pms_exec("INSERT INTO pharmacist (first_name,last_name,staff_id,postal_address,phone,email,username,password,date) VALUES (?,?,?,?,?,?,?,?,NOW())", 'ssssssss', [$first, $last, $staff, $address, $phone, $email, $username, password_hash($password, PASSWORD_DEFAULT)]);
                pms_flash('Đã tạo tài khoản dược sĩ mới.');
            }
        }
    }

    if (!$errors) pms_redirect('admin_pharmacist.php');
    $editing = $id > 0;
}

$record = ['pharmacist_id' => '', 'first_name' => '', 'last_name' => '', 'staff_id' => '', 'postal_address' => '', 'phone' => '', 'email' => '', 'username' => ''];
if ($editing) $record = pms_fetch_one("SELECT * FROM pharmacist WHERE pharmacist_id = ?", 'i', [$id]) ?: $record;
$list = pms_fetch_all("SELECT * FROM pharmacist ORDER BY date DESC, pharmacist_id DESC");
pms_render_header('Admin • Dược sĩ', 'admin', 'admin_pharmacist.php', ['Tổng dược sĩ' => count($list)]);
?>
<section class="panel">
    <div class="panel-head"><h2><?= $editing ? 'Chỉnh sửa dược sĩ' : 'Tạo tài khoản dược sĩ mới' ?></h2></div>
    <?php if ($errors): ?><div class="alert error"><?= pms_h(implode(' ', $errors)) ?></div><?php endif; ?>
    <form method="post" class="form-grid two">
        <input type="hidden" name="id" value="<?= (int)($record['pharmacist_id'] ?? 0) ?>">
        ...
        <div class="actions" style="grid-column:1/-1;">
            ...
            <?php if ($editing): ?><a class="btn secondary" href="admin_pharmacist.php">Hủy chỉnh sửa</a><?php endif; ?>
        </div>
    </form>
</section>
<section class="panel">
    <div class="panel-head"><h2>Danh sách dược sĩ</h2></div>
            ...
            <tbody>
                <?php foreach ($list as $row): ?>
                    <tr>
                        ...
                        <td><div class="actions"><a class="btn secondary" href="admin_pharmacist.php?edit=<?= (int)$row['pharmacist_id'] ?>">Sửa</a><a class="btn danger" href="delete_pharmacist.php?pharmacist_id=<?= (int)$row['pharmacist_id'] ?>" onclick="return confirm('Xóa dược sĩ này?')">Xóa</a></div></td>
                    </tr>
                <?php endforeach; ?>
                <?php if (!$list): ?><tr><td colspan="6"><div class="empty">Chưa có tài khoản dược sĩ nào.</div></td></tr><?php endif; ?>
            </tbody>
        </table>
    </div>
</section>
